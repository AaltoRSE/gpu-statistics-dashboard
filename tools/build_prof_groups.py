...
def _description_matches(unit, prof, surname_counts):
    ...
    The surname must appear (diacritics stripped, case-folded). When
    several professors share the surname (Kaski, Hämäläinen, ...), the
    description must name the given name too, so Kaski Sami and Kaski
    Petteri cannot claim each other's units.
    """
    ...
    for desc in unit["descriptions"]:
        ...
        if sn not in text:
            continue
        if ambiguous and given and given not in text:
            continue
        return True
    return False
...
def school_of(prof, dept_code):
    ...
    company/division text, else the department code's own prefix
    ('T410' -> 'T4') with no full name."""
    ...
    if folded:
        for keyword, short in SCHOOL_KEYWORDS:
            if keyword.casefold() in folded:
                return short, text
    return (dept_code[:2] if dept_code else ""), ""
...
def build(professors_text, unit_groups_text, nss=None):
    ...
    for prefix, school in sorted(schools.items()):
        lines.append(("%s = %s | %s" % (
            prefix, school["short"], school["full"])).rstrip())
    lines += ["", "[departments]"]
...
def _report(profs, groups, units, claimed):
    ...
    if no_unit:
        ...
        for user, g in no_unit:
            extra = "" if g["in_nss"] else "  [not in NSS — no dept either]"
