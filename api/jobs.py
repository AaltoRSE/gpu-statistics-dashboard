...
from domain.jobs import (
    ...
    fetch_job_window,
    sort_by_effective_hours,
    unmonitored_running_jobs,
)
...
def api_jobs(
    ...
    user = user.strip()
    if refresh:
        # Forced refresh bypasses the app's 60-second window cache, the
        # Prometheus client's 20/60 s response cache, and the 30 s
        # controller snapshot (which is what surfaces newly started jobs
        # on nodes whose exporter publishes nothing) instead of redrawing
        # the same data; it also forces a fresh live-ID query.
        deps.get_prom().clear_cache()
        deps.route_cache.invalidate(
            cache.job_window_key(since_hours, True, user or None),
            cache.scontrol_jobs_key())
    if running_only:
        # Live-ID check first: with no running GPU jobs we must not issue
        # the broad window range query at all. An empty live set no longer
        # settles that on its own — scontrol may still report running GPU
        # jobs whose exporters are silent — so the (cached) controller
        # snapshot has to agree there is nothing before we skip the query.
        live = running_gpu_job_ids()
        if not live and not unmonitored_running_jobs(set(), user=user or None):
    ...
    # Running GPU jobs whose node exporter publishes nothing are invisible
    # to the Prometheus-only discovery above; scontrol still sees them, so
    # they are merged in here (unmeasured, flagged monitored=False) rather
    # than silently dropped from the cluster's picture.
    extra = unmonitored_running_jobs({j["jobid"] for j in jobs},
                                     user=user or None)
    if extra:
        jobs = sort_by_effective_hours(jobs + extra)
    ...
    if running_only:
        # An unmonitored job is running by definition (scontrol reported it
        # RUNNING), so it qualifies without a live Prometheus series.
    ...
    #
    # Unmeasured jobs are exempt from the cap. They have no effective
    # GPU-hours to rank by, so they sort last and any cap smaller than the
    # candidate set would hide every one of them — which is precisely the
    # blind spot they exist to close. Their number is bounded by how much
    # running work sits on non-reporting nodes, so this cannot balloon the
    # response the way lifting the cap generally would.
    if not running_only:
        jobs = ([j for j in jobs if j.get("monitored", True)][:limit]
                + [j for j in jobs if not j.get("monitored", True)])
    enrich(jobs)
    if search:
        needle = search.lower()
        jobs = [
